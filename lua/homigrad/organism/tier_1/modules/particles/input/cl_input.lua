...
local function impact(pos,vel,mul)
	...
	-- Blood spray com spread aumentado
	addBloodPart2(pos + velnorm, -vel * 1.0 + Vector(Rand(-40, 40), Rand(-40, 40), Rand(-20, 20)) * 0.5, nil, 25, 25, 0.3)
	addBloodPart2(pos + velnorm, -vel * 0.7 + Vector(Rand(-35, 35), Rand(-35, 35), Rand(-15, 15)) * 0.5, nil, 25, 25, 0.3)
	addBloodPart2(pos + velnorm, -vel * 0.5 + Vector(Rand(-30, 30), Rand(-30, 30), Rand(-10, 10)) * 0.5, nil, 25, 25, 0.3)
	addBloodPart2(pos + velnorm, -vel * 0.3 + Vector(Rand(-25, 25), Rand(-25, 25), Rand(-5, 5)) * 0.5, nil, 25, 25, 0.3)

	-- Partículas de sangue com spread ainda maior
	for i = 1, iters do
		local size = 1
		local spreadMult = math.random(30, 60)
		addBloodPart(pos, -vel * (i / iters) * 0.8 + Vector(Rand(-spreadMult, spreadMult), Rand(-spreadMult, spreadMult), Rand(-spreadMult * 0.3, spreadMult * 0.3)), mat_huy, size, size, false, false)
	end
end

net.Receive("hg_bloodimpact", function()
	local pos = net.ReadVector()
	local vel = net.ReadVector() * math.random(200, 500)
...
hook.Add("HG_OrganismChanged", "explodelegs", function(oldorg, org)
	...
	for ind, nam in pairs(limbs) do
		local key = ind.."amputated"
		if not oldorg[key] and org[key] then
