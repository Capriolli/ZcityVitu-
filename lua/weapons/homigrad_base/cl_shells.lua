...
function SWEP:MakeShell(shell, pos, ang, vel)
	...
	-- Velocidade variada com componente aleatório
	local variedVel = vel + (((IsValid(self) and IsValid(self:GetOwner())) and self:GetOwner():GetVelocity()/1.1) or Vector(0,0,0))
	variedVel = variedVel + VectorRand() * math.random(30, 80) -- Variação de velocidade
	variedVel = variedVel * math.Rand(0.7, 1.3) -- Multiplicador aleatório de velocidade
	
	phys:SetVelocity(variedVel)
	
	-- Ângulo de rotação variados
	local randomSpin = math.random(1500, 4500) -- Maior variação no spin
	local randomDir = VectorRand() * randomSpin
	local forwardSpin = ang:Forward() * math.random(1500, 3500)
	phys:SetAngleVelocity(randomDir - forwardSpin)
